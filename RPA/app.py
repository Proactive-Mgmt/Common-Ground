...
def initialize_driver():
    ...
    print(f"headless:{selenium_options["headless"]}")
...
def get_appointments (driver): 
    schedule_url: LiteralString = f"https://static.practicefusion.com/apps/ehr/index.html?utm_source=exacttarget&utm_medium=email&utm_campaign=InitialSetupWelcomeAddedUser#/PF/schedule/scheduler/agenda"
...
# Call this function before interacting with elements that might trigger alerts
    ...
    button = WebDriverWait(driver, 10).until(  EC.element_to_be_clickable(  (By.XPATH, "//button[@class='btn--default' and @data-element='btn-schedule-print']")  )  )    
    button.click()

    time.sleep(3)    
    
    # Find the HTML element representing the table
    table = driver.find_element(By.CSS_SELECTOR, 'table[data-element="table-agenda-print"]')
    ...
    for row in rows[1:]:
        ...
        for col in cols:
            ...
            if contact_details:
                details = [detail.text.strip() for detail in contact_details[0].find_elements(By.TAG_NAME, "div")]
